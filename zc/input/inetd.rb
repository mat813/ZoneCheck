...
module Input
	...
	def interact(p, c, tm)
	    ...
	    puts 
	    puts "For more information visit about ZoneCheck visit the"
	    puts "following URL:"
		...
		when /^set\s+(\w+)\s+(.*)$/
		    ...
		    when 'ns'		then p.domain.ns	= $2
		    when 'zone','domain' then p.domain.name	= $2
		    end
		# Unset
		when /^unset\s+(\w+)$/
		    case $1
...
EOT
		...
		when /^(?:quit|q|exit)$/	then return false
		    ...
		    puts "I didn't understand, try asking for help!"
		end
