...
\author{Stéphane Bortzmeyer\\AFNIC ("\texttt{.fr}" registry)\\\texttt{bortzmeyer@nic.fr}}
...
\begin{frame}
\frametitle{Why testing a DNS zone?}

\begin{itemize}
\item<2->To be sure it works,
\item<3->To be sure it works fast (no timeouts or retransmissions).
\end{itemize}

\begin{block}<4>{It is not because ``it works'' that everything is perfect.}{}\end{block}
\end{frame}

\begin{frame}
  \frametitle{The requirments}
When we started designing the new Zonecheck in 2002 (version 2, a program by
...
\item<4->Quite general tool, not a small ad hoc hack,
...
\item<6->Used in daily production at AFNIC since.
\end{itemize}
\end{frame}
...
\only<2->{The code defines the tests you \emph{can} run, the
  configuration file defines the tests you \emph{do} run and their
...
\only<2-3>{The policy is quite strict. A few examples:
  \begin{itemize}
    ...
    wether the loopback address is delegated in in-addr.arpa).
  \end{itemize}
}

\only<3->{
  This creates a lot of support tickets when delegating and allow us
  to measure the current level of knowledge of some registrars :-)
}

\only<4->{But it makes a much better zone and strongly diminishes the
  post-registration complaints ``My site does not work''.}
\end{frame}
...
\only<2->{Many registries asked that such tests must be clearly
  described, and executed in a predictable way. An automatic tool,
  such as Zonecheck, allows to fulfill these requirments.}
